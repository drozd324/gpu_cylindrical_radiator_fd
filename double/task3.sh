...
for (( i=0; i<$NUM_POWERS; i++ )); do
	for (( j=i; j<$NUM_POWERS; j++ )); do
			...
			#TEMP=$(./task2 -m ${POWERS[$j]} -n ${POWERS[$j]} -p $NUM_ITER -x $i -y $i -s)

		echo "./task2 -m ${POWERS[$j]} -n ${POWERS[$j]} -p $NUM_ITER -x ${POWERS[$i]} -y ${POWERS[$i]} -s"
		#echo "./task2 -m ${POWERS[$j]} -n ${POWERS[$j]} -p $NUM_ITER -x $i -y $i -s"
